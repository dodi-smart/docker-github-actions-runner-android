...
> **Fork note (dodi-smart):** inherited from upstream
> `compscidr/docker-github-actions-runner-android` as a record of the original
> design. This fork implements the PR build gate (`build.yml`) but *not* the
> `AUTOMERGE_PAT` / auto-merge half, and its build matrix additionally covers
> Node majors and a `linux/arm64` build. See the README for what actually runs
> here.
